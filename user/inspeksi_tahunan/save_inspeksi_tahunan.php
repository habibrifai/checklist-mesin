...
mysqli_query($conn, "INSERT INTO form(`no_form`, `tanggal`, `nip_pemeriksa`, `nip_pj`, `periode`) VALUES ('$no_form','$tanggal','$nip_pemeriksa','$nip_pj','Tahunan');");

for ($i=35; $i < 42; $i++) { 
	$kondisi = $_POST['radio'.''.$i];
	$keterangan = $_POST['hasilpemhb'.''.$i];
	$rekomendasi = $_POST['rekomhb'.''.$i];

	$insert = mysqli_query($conn, "INSERT INTO hasil(`no_form`, `no_equipment_data`, `kondisi`, `keterangan`, `rekomendasi`) VALUES ('$no_form','$i','$kondisi','$keterangan','$rekomendasi');");
}
