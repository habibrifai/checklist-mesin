...
class myPDF extends FPDF{
	...
	function Header()
	{
		...
		$this->Cell(50);
		$this->Cell(170,8,'CHECK LIST BULANAN/SHUTDOWN/START UP',1,0,'C');
...
while($baris = mysqli_fetch_array($data_inspeksi)){ 
	...
	if ($baris['no_equipment_data'] == 18) {
		$jenis = '  Aligment motor & gear box';
	} 
	if ($baris['no_equipment_data'] == 19) {
		$jenis = '  Aligment bearing';
	}
	if ($baris['no_equipment_data'] == 20) {
		$jenis = '  Inlet, outlet, & inside granulator';
	}
	if ($baris['no_equipment_data'] == 21) {
		$jenis = '  Connect motor & Running test';
	}
	if ($baris['no_equipment_data'] == 22) {
		$jenis = '  Pelumas riding ring & Riding gear';
	}
	if ($baris['no_equipment_data'] == 23) {
		$jenis = '  Instrumentasi control valve';
	}
	if ($baris['no_equipment_data'] == 24) {
		$jenis = '  Turnion, pinion, & trust roll';
	}
	if ($baris['no_equipment_data'] == 25) {
		$jenis = '  Labyrinth seal inlet';
	}
	if ($baris['no_equipment_data'] == 26) {
		$jenis = '  Sparger';
	}
	if ($baris['no_equipment_data'] == 27) {
		$jenis = '  Line / nozzle SA';
	}
	if ($baris['no_equipment_data'] == 28) {
		$jenis = '  Line / nozzle NH3';
	}
	if ($baris['no_equipment_data'] == 29) {
		$jenis = '  Line / nozzle Steam';
	}
	if ($baris['no_equipment_data'] == 30) {
		$jenis = '  Line / nozzle slurry';
	}
	if ($baris['no_equipment_data'] == 31) {
		$jenis = '  Ploughshare';
	}
	if ($baris['no_equipment_data'] == 32) {
		$jenis = '  Rubber panel';
	}
	if ($baris['no_equipment_data'] == 33) {
		$jenis = '  Caping';
	}
	if ($baris['no_equipment_data'] == 34) {
		$jenis = '  Blast aerator & instrumentasi plant air';
	}
